    ...
    if 'next_screen' not in st.session_state:
        ...
        if os.path.exists(setting_csv_path):
            ...
            if not df.empty:
                ...
                # メッセージを表示
                st.markdown(f"<p style='text-align: right; font-size: 2em; font-weight: bold; color: navy; text-shadow: 2px 2px 5px rgba(0, 0, 0, 0.3);'>{message}</p>", unsafe_allow_html=True)
